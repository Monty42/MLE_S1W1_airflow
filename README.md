...
2. На основе ETL-пайплайна реализуйте таски преобразования (`transform`) и выгрузки данных (`load`)
    ...
    <summary>Если выполняете данный пункт больше 5 минут, загляните сюда</summary>

    На самом деле весь код уже написан в файле `wb_data_etl`. Займитесь копипастом. В качестве изменения необходимо указать декораторы, а также загрузить данные в БД с помощью хуков (пример есть в функции `extract`).
